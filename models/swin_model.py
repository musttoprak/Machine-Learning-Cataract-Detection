...
for fold, (train_index, val_index) in enumerate(kfold.split(X), 1):
    ...
    num_features = swin_model.head.fc.out_features 
    num_classes = 2 
    swin_model.classifier = nn.Linear(num_features, num_classes)
    swin_model.dropout = nn.Dropout(p=0.5)
    optimizer = optim.SGD(swin_model.parameters(), lr=0.0001, momentum=0.9, weight_decay=0.01)
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.1)
      
    # Modeli eğitme ve değerlendirme
    train_losses, val_losses, val_accs, val_f1_scores, val_recalls, val_precisions, val_sensitivities, val_specificities, val_mcc_scores, val_aucs, val_fpr, val_tpr = train_and_evaluate_model(swin_model, criterion, optimizer, train_loader, val_loader)
    ...
    torch.save(fold_results, f"results/swin/swin_fold{fold}.pth")
...
# Sonuçları bir dosyaya kaydetme
torch.save(all_results, "results/swin/swin_all.pth")