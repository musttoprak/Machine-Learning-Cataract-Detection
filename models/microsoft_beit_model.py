...
for fold, (train_index, val_index) in enumerate(kfold.split(X), 1):
    ...
    num_features = beit_model.classifier.in_features  # Başlık katmanının giriş özellik sayısını alın
    num_classes = 2  # İki sınıf: Normal ve Katarakt
    beit_model.classifier = nn.Linear(num_features, num_classes)
    beit_model.dropout = nn.Dropout(p=0.7)
    #optimizer = optim.Adam(beit_model.parameters(), lr=0.000001, momentum=0.99, weight_decay=0.1)
    #optimizer = optim.Adagrad(beit_model.parameters(), lr=0.000001, weight_decay=0.1)
    optimizer = optim.RMSprop(beit_model.parameters(), lr=0.000001, alpha=0.99, eps=1e-08, weight_decay=0.1, momentum=0.99)
    #Adam, AdaGrad,RMSProp
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.1)
    
    # Modeli eğitme ve değerlendirme
    train_losses, val_losses, val_accs, val_f1_scores, val_recalls, val_precisions, val_sensitivities, val_specificities, val_mcc_scores, val_aucs, val_fpr, val_tpr = train_and_evaluate_model(beit_model, criterion, optimizer, train_loader, val_loader)
    ...
    torch.save(fold_results, f"results/microsoft_beit/microsoft_beit_fold{fold}.pth")
...
for fold in range(1, kFoldNumber+1):
    fold_results = torch.load(f"results/microsoft_beit/microsoft_beit_fold{fold}.pth")
    for key in all_results:
        all_results[key].extend(fold_results[key])

# Sonuçları bir dosyaya kaydetme
torch.save(all_results, "results/microsoft_beit/microsoft_beit_all.pth")
