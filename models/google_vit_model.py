...
for fold, (train_index, val_index) in enumerate(kfold.split(X), 1):
    ...
    train_losses, val_losses, val_accs, val_f1_scores, val_recalls, val_precisions, val_sensitivities, val_specificities, val_mcc_scores, val_aucs, val_fpr, val_tpr = train_and_evaluate_model(google_vit_model, criterion, optimizer, train_loader, val_loader)
    ...
    torch.save(fold_results, f"results/google_vit/google_vit_fold{fold}.pth")
...
for fold in range(1, kFoldNumber+1):
    fold_results = torch.load(f"results/google_vit/google_vit_fold{fold}.pth")
    for key in all_results:
        all_results[key].extend(fold_results[key])

# Sonuçları bir dosyaya kaydetme
torch.save(all_results, "results/google_vit/google_vit_all.pth")